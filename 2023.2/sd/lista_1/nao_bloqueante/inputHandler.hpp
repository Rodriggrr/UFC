    ...
    while(true){
        ...
        if(args[0] == "!list"){
            for (int i = 0; i < connections.size(); i++){
                std::cout << i << ": " << connections[i]->getClient().getIpAddress() << ":" << connections[i]->getClient().getPort() << " (" << connections[i]->getClient().getHost() << ")" << std::endl;
            }
        }
            ...
            std::cout << "Comandos disponíveis:\n"
                      ...
                      << "!msg <index> <text> - envia uma mensagem a um usuario especifico.\n"
